...
foreach my $protocol (qw(4)) {
	# XXX-SET-ME
	my $conffile = "/usr/local/etc/bird.conf";
	my $sockfile = "/usr/local/var/run/bird.ctl";
	...
	#open (INPUT, '/usr/local/sbin/birdc -s '.$sockfile.' show protocols |');
	#while (<INPUT>) {
	...
	foreach my $asn (@asnlist) {
		...
		#open (INPUT, '/usr/local/sbin/birdc -s '.$sockfile.' show route |') or die "Error: $!\n";
		open(INPUT, "< /home/skims/git/ixp/tools/rib-dump.txt") or die "Error: $!\n";
		while (<INPUT>) {
